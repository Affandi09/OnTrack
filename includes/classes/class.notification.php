...
class Notification extends App
{
  ...
  public static function ticketUser($ticketid, $reply, $templateid)
  { //send ticket notification
    ...
    if ($ticket['departmentid'] != 0)
      $department = getSingleValue("tickets_departments", "name", $ticket['departmentid']);
    ...
    if (!empty($ticket['branch_id']))
      $b = getSingleValue("branches", "name", $ticket['branch_id']);
    if (!empty($b))
      $branch_name = $b;
    if (!empty($ticket['submitter_id']))
      $s = getSingleValue("submitters", "name", $ticket['submitter_id']);
    if (!empty($s))
      $submitter_name = $s;

    // {ticketid}, {subject}, {message} dikosongkan karena sudah tampil di tabel detail tiket
    $search  = array('{ticketid}', '{status}', '{subject}', '{contact}', '{message}', '{company}', '{appurl}', '{client}', '{department}');
    $replace = array('', $ticket['status'], '', $contact, '', getConfigValue("company_name"), getConfigValue("app_url"), $client, $department);

    $subject = str_replace($search, $replace, $template['subject']);
    $message = str_replace($search, $replace, $template['message']);

    // Prepend ticket detail block so it appears above the template message body
    $message = self::ticketInfoBlock($ticket['ticket'], $ticket['subject'], $contact, $submitter_name, $branch_name, $ticket['priority'], $ticket['timestamp']) . $message;
  ...
  public static function ticketStaff($ticketid, $reply, $templateid)
  { //send ticket notification
    global $database;
    $template = getRowById("notificationtemplates", $templateid);
    $ticket = getRowById("tickets", $ticketid);
  ...
  private static function ticketInfoBlock($ticketno, $subject, $contact, $submitter, $branch, $priority, $timestamp)
  {
...
<table style="border-collapse:collapse; width:100%; max-width:600px; font-family:Arial,sans-serif; font-size:14px; margin-top:12px; border:1px solid #dee2e6; border-radius:6px; overflow:hidden;">
  ...
  <tbody>
    <tr style="background-color:#f8f9fa;">
      <td style="padding:8px 14px; font-weight:bold; width:35%; color:#495057;">No Tiket</td>
      <td style="padding:8px 14px; color:#212529;">' . htmlspecialchars($ticketno) . '</td>
    </tr>
    <tr style="background-color:#f8f9fa;">
...
';
  ...
  public static function newUser($peopleid, $password)
  ...
  public static function passwordReset($peopleid, $resetlink)
  ...
  public static function monitoringEmail($peopleid, $hostid, $hostinfo, $status)
  ...
  public static function monitoringSMS($peopleid, $hostid, $hostinfo, $status)
